  ...
  return (
    <label className={cn('flex flex-col gap-2 text-sm font-medium', isLight ? 'text-slate-600' : 'text-slate-200', labelClassName)}>
      ...
      <div className="relative">
        <select
          className={cn(
            isLight
              ...
              : 'h-11 w-full appearance-none rounded-[0.95rem] border border-white/10 bg-white/[0.06] px-3 pr-10 text-sm text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] outline-none transition placeholder:text-slate-400 focus:border-[#8f9cff] focus:ring-4 focus:ring-[#8f9cff]/15 disabled:cursor-not-allowed disabled:opacity-60',
            className
          )}
          {...props}
        >
        ...
        <span className={cn('pointer-events-none absolute inset-y-0 right-3 flex items-center', isLight ? 'text-slate-500' : 'text-slate-300')}>
